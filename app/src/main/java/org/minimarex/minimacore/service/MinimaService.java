...
public class MinimaService extends Service {
    ...
    public static volatile SimpleDateFormat DATEFORMAT       = new SimpleDateFormat("HH:mm:ss dd/MM/yyyy", Locale.ENGLISH);
    public static volatile SimpleDateFormat DATEFORMAT_TIME  = new SimpleDateFormat("HH:mm:ss", Locale.ENGLISH);
    ...
    private MinimaService mService;

    private MinimaServiceListener mServiceListener = null;

    public MinimaServiceListener getServiceListener() {
        return mServiceListener;
    }

    public void setServiceListener(MinimaServiceListener listener) {
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            mWifiLock = wifi.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "Minima::MiniWiFi");
        } else {
            ...
            public void processMessage(Message zMessage) throws Exception {
                ...
                if(zMessage.getMessageType().equals(NotifyManager.NOTIFY_POST)){
                    ...
                    if(event.equals("NEWBLOCK")) {
                        ...
                        if(mServiceListener != null) {
                            mServiceListener.MinimaLoadKeys(((Number) data.get("keys")).intValue(), true);
                        }
                    }else if(event.equals("LOAD_ALL_KEYS_VALUE")){
                        //MinimaLogger.log("SERVICE received "+event+" "+data.toString(), false);

                        if(mServiceListener != null) {
                            mServiceListener.MinimaLoadKeys(((Number) data.get("keys")).intValue(), false);
                        }
                    }
                }
            }
    ...
    public void sendBroadcastNotify(String zMessage){
        ...
        for(int i=0;i<tot;i++){
            ...
            if(((Number)app.get("penabled")).intValue() == 1){
                //Get the package
    ...
    private void setMinimaNotification(){
        ...
        if(mTxPowJSON == null){
            ...
            long timemilli  = Long.parseLong(header.getString("timemilli"));
    ...
    public void onDestroy() {
        ...
        try{
            ...
            MinimaLogger.log("ERROR unregistering receiver: "+exc);
        }
